...
exercises the PER-BLOCK attention-head path for tests/TestNeuralPretrained.pas
(TestSDUNetPerBlockHeadsParity).

The diffusers config "attention_head_dim" can be a LIST (one head DIM per
resolution block; SDXL-style) rather than a scalar. HF semantics: the per-head
channel DIM is given, and a block's num_heads = (that block's channel width) /
its head dim. With a list of DIFFERENT dims over different block widths the head
COUNT varies per block.

Here both resolutions are attended (CrossAttnDown/Up on every block) so the
per-block head count is actually exercised:

  block_out_channels = [8, 16]
  attention_head_dim  = [2, 8]    (a LIST)
  -> heads[0] = 8 // 2 = 4
     heads[1] = 16 // 8 = 2       (DIFFERENT from block 0)

Everything else mirrors tools/sd_unet_tiny_fixture.py / the SDXL fixture
EXACTLY (same numpy float64 oracle, same key scheme). diffusers is NOT
installed; the reference forward is the self-contained oracle below.
use_linear_projection is FALSE and transformer_layers_per_block 1 so this
fixture isolates the per-block-heads path.

Coded by Claude (AI).

Usage (from the repo root):
  /home/bpsa/x/bin/python tools/sd_unet_perblock_heads_tiny_fixture.py
writes tests/fixtures/tiny_sd_unet_pbheads{.safetensors,_config.json,_io.json}.
"""
import json
import math
import numpy as np
...
# ---- pico config (both blocks attended, PER-BLOCK head dims) ----
IN_CH = 4
OUT_CH = 4
BLOCK_OUT = [8, 16]
LAYERS_PER_BLOCK = 1
CROSS_DIM = 12
NORM_GROUPS = 4
# attention_head_dim is a LIST -> per-block head COUNTS differ.
ATTN_HEAD_DIM = [2, 8]                       # dims per block
HEADS = [BLOCK_OUT[i] // ATTN_HEAD_DIM[i] for i in range(len(BLOCK_OUT))]  # [4, 2]
LATENT = 8
TEXT_SEQ = 5
TIME_EMBED_DIM = BLOCK_OUT[0] * 4           # 32
# Both down blocks AND both up blocks attend so heads[0] != heads[1] is exercised
# on the down path, mid (uses block 1's heads), and the up path.
DOWN_TYPES = ['CrossAttnDownBlock2D', 'CrossAttnDownBlock2D']
UP_TYPES = ['CrossAttnUpBlock2D', 'CrossAttnUpBlock2D']
...
def forward(latent, t, enc, sd):
    ...
    # mid block uses the LAST (deepest) block's head count.
    h = transformer2d(h, enc, sd, 'mid_block.attentions.0.', mid_ch, HEADS[-1])
    ...
    for i, utype in enumerate(UP_TYPES):
        ...
        for j in range(LAYERS_PER_BLOCK + 1):
            ...
            h = resnet_block(h, temb, sd, f'up_blocks.{i}.resnets.{j}.',
                             resnet_in + res_skip, output_channel)
            if utype == 'CrossAttnUpBlock2D':
                # up block i mirrors resolution block (n-1-i): use that head count.
                h = transformer2d(h, enc, sd, f'up_blocks.{i}.attentions.{j}.',
                                  output_channel, rev_heads[i])
        if i != n - 1:
...
def main():
    print(f'block_out_channels = {BLOCK_OUT}')
    print(f'attention_head_dim = {ATTN_HEAD_DIM}  -> heads per block = {HEADS}')
    assert HEADS[0] != HEADS[1], 'fixture must have DIFFERING per-block head counts'
    ...
    save_file(sd_f32, 'tests/fixtures/tiny_sd_unet_pbheads.safetensors')
    config = {
        ...
        'attention_head_dim': ATTN_HEAD_DIM,     # a LIST (per-block head dims)
    ...
    with open('tests/fixtures/tiny_sd_unet_pbheads_config.json', 'w') as f:
        json.dump(config, f, indent=1)
    with open('tests/fixtures/tiny_sd_unet_pbheads_io.json', 'w') as f:
        json.dump({
    ...
    print(f'wrote tiny_sd_unet_pbheads.safetensors ({len(sd_f32)} tensors) + config + io')

    # ---- fixture self-check: a per-block head count actually MATTERS ----
    # Re-run the oracle treating block-0 as if it had block-1's head count; the
    # output MUST change (otherwise the per-block path is not being exercised).
    base = noise.copy()

    def forward_wrong_heads(lat, tt, e, s):
        # monkeypatch: force HEADS[0] := HEADS[1] for a single re-run.
        global HEADS
        saved = HEADS
        HEADS = [HEADS[1], HEADS[1]]
        try:
            return forward(lat, tt, e, s)
        finally:
            HEADS = saved

    alt = forward_wrong_heads(latent, t, enc, sd)
    d = np.abs(alt - base).max()
    assert d > 1e-4, f'per-block head count had no effect ({d})'
    print(f'per-block head count effect (block0 4->2 heads): max|diff| = {d:.4f}')
