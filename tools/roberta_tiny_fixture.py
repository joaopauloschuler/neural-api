#!/usr/bin/env python3
"""Generate a tiny RANDOM RoBERTa parity fixture for
...
One fixture, ~14 KB, pinned in tests/fixtures/:

  tiny_roberta.*: RobertaModel (model_type "roberta") with the traits that
      distinguish RoBERTa from the landed BERT importer:
        - THE position-id OFFSET: create_position_ids_from_input_ids
          starts real-token positions at padding_idx+1 = pad_token_id+1
          = 2, so checkpoint position rows 0 and 1 are NEVER read and the
          usable context is max_position_embeddings - 2 (the
          514-rows-for-512-positions layout). The script ASSERTS rows 0/1
          are unused AND that an importer reading rows 0..L-1 instead of
          2..L+1 (the natural bug) fails the 2e-5 Pascal parity gate;
        - type_vocab_size = 1: the token-type branch degenerates to a
          single constant row added at every position;
        - otherwise the exact BERT skeleton: same tensor names (RobertaModel
          exports unprefixed), POST-LN blocks, bidirectional attention,
          exact erf GELU (asserted visible), pooler head.

The reference is computed by HF transformers in float64 (the oracle
convention of the committed fixtures): last_hidden_state + pooler_output
for every sequence. Sequences are MAX_POS-2 = 14 tokens long (the full
usable context) and contain NO pad token (id 1) - a pad token would get a
padding position id and break the consecutive-positions assumption of the
importer.

Coded by Claude (AI).

Usage (from the repo root):
  python3 tools/roberta_tiny_fixture.py
writes tests/fixtures/tiny_roberta{.safetensors,_config.json,_hidden.json}.
...
model = RobertaModel(RobertaConfig(**cfg_dict, attn_implementation='eager'))
# Same pico-width boosts as tools/bert_tiny_fixture.py: O(1)-structured
...
save_file(sd, 'tests/fixtures/tiny_roberta.safetensors')
with open('tests/fixtures/tiny_roberta_config.json', 'w') as f:
...
with open('tests/fixtures/tiny_roberta_hidden.json', 'w') as f:
    json.dump({'sequences': sequences, 'token_types': token_types,
               'hidden': hidden, 'pooler': pooler}, f)
print(f'wrote tiny_roberta.safetensors ({len(sd)} tensors) '
...
with torch.no_grad():
    ...
    # reference at all.
    scrub = RobertaModel(RobertaConfig(**cfg_dict,
    ...
    bug = RobertaModel(RobertaConfig(**cfg_dict,
                                     attn_implementation='eager'))
    ...
    tanh_cfg = RobertaConfig(**{**cfg_dict,
                                'hidden_act': 'gelu_pytorch_tanh'},
                             attn_implementation='eager')
    tanh_model = RobertaModel(tanh_cfg)
