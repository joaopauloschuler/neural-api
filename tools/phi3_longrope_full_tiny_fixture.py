#!/usr/bin/env python3
"""Generate a tiny RANDOM Phi-3 LongRoPE parity fixture with FULL rotary for
tests/TestNeuralPretrained.pas (no network: the model is randomly
initialized from a pico config, never downloaded).

The sibling tools/phi3_longrope_tiny_fixture.py covers longrope over a PARTIAL
rotary (partial_rotary_factor 0.75). This one omits partial_rotary_factor
entirely - exactly as the released Phi-3-mini-128k configs do - so the reader
takes its 1.0 default and rotary_dim = head_dim. That combination selects a
different import path (the whole head rotates, so the builder is free to hoist
one rotary layer ahead of the head split) and long_factor then has head_dim/2
entries instead of rotary_dim/2.
...
variants) import path on the Llama backbone:
  ...
  - the long attention scaling sqrt(1 + ln(factor)/ln(orig)) with
    ...
    applied to cos/sin.
head_dim = hidden/num_heads = 8, FULL rotary -> rotary_dim = 8 ->
long_factor / short_factor have head_dim/2 = 4 entries.

Reference logits are computed by HF transformers in float64.

Usage (from the repo root):
  /home/bpsa/x/bin/python tools/phi3_longrope_full_tiny_fixture.py
writes tests/fixtures/tiny_phi3_longrope_full{.safetensors,_config.json,
_logits.json}.
...
N_HEAD = 4
N_KV_HEAD = 2
D_MODEL = 32            # head_dim = 8; FULL rotary -> rotary_dim = 8
D_FF = 24
...
# head_dim/2 = 4 entries; non-trivial (> 1) so the per-frequency division
# changes the angles meaningfully.
SHORT_FACTOR = [1.0, 1.0, 1.0, 1.0]
LONG_FACTOR = [1.0, 1.3, 1.8, 2.4]
...
phi3_cfg = {
    ...
    'rms_norm_eps': 1e-5,
    'rope_theta': 10000.0,
    # NO partial_rotary_factor: the reader's 1.0 default -> full rotary.
    'rope_scaling': {
        'rope_type': 'longrope',
...
save_file(sd_saved, 'tests/fixtures/tiny_phi3_longrope_full.safetensors')
with open('tests/fixtures/tiny_phi3_longrope_full_config.json', 'w') as f:
...
with open('tests/fixtures/tiny_phi3_longrope_full_logits.json', 'w') as f:
    json.dump({'sequences': sequences, 'logits': logits}, f)
print(f'wrote tiny_phi3_longrope_full.safetensors ({len(sd_saved)} tensors) '
