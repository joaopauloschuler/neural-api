#!/usr/bin/env python3
"""Generate a tiny RANDOM torchvision-EfficientNet parity fixture for
tests/TestNeuralPretrained.pas (no network access, no torchvision/timm needed:
the model is a pico random-init EfficientNet whose state_dict mirrors
torchvision's exact key scheme, and the reference logits are computed by a
self-contained numpy float64 oracle).

Neither torchvision nor timm is installed in the reusable venv, so - mirroring
tools/mobilenetv3_tiny_fixture.py - the oracle is hand-written in numpy. The
pico net is config-faithful to a SHRUNK efficientnet_b0: the per-block expand-
ratio / kernel / stride / SE-ratio table is read from the same JSON the Pascal
importer reads, so b1..b7 are reachable by editing the config alone.

EfficientNet vs MobileNetV3 (the only deltas - same MBConv primitives):
  - EVERY activation (stem / expand / depthwise / head) is SiLU/swish
    (x * sigmoid(x)), NOT MobileNetV3's alternating ReLU / hard-swish.
  - squeeze-excite uses SiLU on the reduced branch + a PLAIN sigmoid gate,
    NOT MobileNetV3's ReLU + hard-sigmoid.
  - the SE reduced dim is round(in_channels * se_ratio) measured against the
    block INPUT channels (not the expanded channels) - baked into se_channels.
  - the head is conv 1x1 +BN +SiLU -> global avg pool -> a SINGLE Linear
    (classifier.1), NOT MobileNetV3's two-Linear classifier.
  - stochastic depth is a forward NO-OP at inference (ignored).

This exercises EVERY branch of the BuildEfficientNet importer
(neuralpretrained.pas):
  - stem conv 3x3 stride2 pad1 + folded BN + SiLU
  - MBConv blocks covering all branch combos:
      * expand_ratio == 1 -> NO expand (depthwise sub-block is .0)
      * expand_ratio  > 1 -> expand .0, depthwise .1
  ...
  - head conv 1x1 + folded BN + SiLU -> global avg pool -> classifier.1 Linear

torchvision Conv2d has bias=False everywhere except the SE 1x1 convs (bias);
every conv is followed by a BatchNorm2d with eps = 1e-3 (torchvision
EfficientNet, NOT 1e-5). The importer FOLDS each BN into the preceding conv at
load time; this oracle does the SAME fold then runs the folded conv.

state_dict key scheme (torchvision efficientnet):
  ...
  classifier.1.{weight,bias}           Linear (head_conv -> num_labels)

POOLING parity note: CAI's TNNetAvgChannel divides by width*width, which equals
a true global mean ONLY when the map is SQUARE. This fixture keeps every
spatial map square (square image, stride-preserving pad = k//2) so AvgChannel
is an exact global average; the oracle uses .mean() over H*W to match.

Coded by Claude (AI).

Usage (from the repo root):
  /home/bpsa/x/bin/python tools/make_pico_efficientnet_fixture.py
writes tests/fixtures/tiny_efficientnet{.safetensors,_config.json,_logits.json}.
...
EPS = 1e-3                  # torchvision EfficientNet BatchNorm2d eps
# Image 32 (square) keeps every spatial map square AND >= the largest depthwise
# kernel (5) after the single stride-2 downsample.
...
# (kernel, exp, out, stride, se, se_channels)
# blk0: expand_ratio 1 -> NO expand (exp==in==STEM=8), SE, stride1, in!=out
# blk1: expand, NO SE, stride2 (downsample)
# blk2: expand, SE, stride1, in==out -> RESIDUAL, kernel 5
# blk3: expand, SE, stride1, in!=out (no residual)
BLOCKS = [
    dict(kernel=3, exp=8,  out=12, stride=1, se=True,  se_channels=2),
    dict(kernel=3, exp=24, out=16, stride=2, se=False, se_channels=0),
    dict(kernel=5, exp=32, out=16, stride=1, se=True,  se_channels=4),
    dict(kernel=3, exp=32, out=20, stride=1, se=True,  se_channels=4),
]
...
def build_state_dict():
    ...
    sd['classifier.1.weight'] = randn(NUM_CLASSES, HEAD_CONV, std=0.3)
    sd['classifier.1.bias'] = randn(NUM_CLASSES, std=0.2)
    return sd
...
def dwconv_bn(x, sd, conv_prefix, bn_prefix, stride, pad):
    ...
    # SHIFT through a following TNNetChannelBias; net result equals folding both
    # into a biased depthwise. We fold both here for parity.
...
def se_module(x, sd, prefix):
    """Squeeze-excite on x:[C,H,W]. pool -> 1x1 reduce -> SiLU -> 1x1 expand ->
    SIGMOID -> per-channel multiply. The 1x1 convs are plain matmuls on the
    ...
    reduced = silu(w1 @ pooled + b1)                     # [se]
    w2 = sd[prefix + '.fc2.weight'][:, :, 0, 0]          # [C, se]
    b2 = sd[prefix + '.fc2.bias']
    gate = sigmoid(w2 @ reduced + b2)                    # [C]
...
def forward(x, sd):
    ...
    logits = sd['classifier.1.weight'] @ pooled + sd['classifier.1.bias']
...
def main():
    ...
    save_file(sd_f32, 'tests/fixtures/tiny_efficientnet.safetensors')
    ...
    with open('tests/fixtures/tiny_efficientnet_config.json', 'w') as f:
        json.dump(config, f, indent=1)
    with open('tests/fixtures/tiny_efficientnet_logits.json', 'w') as f:
        json.dump({
            'pixels': pixels.tolist(),            # [3][IMAGE][IMAGE]
            'logits': [logits.tolist()],          # [1][NUM_CLASSES]
            'num_labels': NUM_CLASSES,
        }, f)
    print(f'wrote tiny_efficientnet.safetensors ({len(sd_f32)} tensors) + '
          f'config + oracle')
    ...
    # residual block blk2 project (has expand .0, dw .1, SE .2, project .3)
    d = effect('features.3.block.3.0.weight')
    assert d > TH, f'blk2 project had no effect ({d})'
    print(f'blk2 project effect: {d:.6f}')
    d = effect('classifier.1.bias')
    assert d > TH, f'classifier.1 bias had no effect ({d})'
    print(f'classifier.1 bias effect: {d:.6f}')
