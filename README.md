...
* [Sub-Pixel Super Resolution](https://github.com/joaopauloschuler/neural-api/tree/master/examples/SubPixelSuperRes) - A tiny `TNNetPixelShuffle` (depth-to-space) head learns a 2x upscaling on synthetic tiles (test PSNR 4 dB → 39 dB), pure CPU
* [Edge of Stability](https://github.com/joaopauloschuler/neural-api/tree/master/examples/EdgeOfStability) - Reproduces "progressive sharpening" under full-batch gradient descent: the top Hessian eigenvalue rises to ~`2/η` and hovers there (Cohen et al. 2021), measured online with `TNNet.HessianCurvatureReport`
...
* [Sequence-Mixer Bake-off](https://github.com/joaopauloschuler/neural-api/tree/master/examples/SequenceMixerBakeoff) - Four token-mixing primitives head-to-head on the *same* tiny char-level next-token task, sharing an identical embedding front-end, per-token MLP read-out, data and init — the only difference is *how information moves between positions*: `TNNetTokenShift` (lag-1, O(n)), `TNNetCausalConv1D` (K=5 causal conv, O(n·K)), `TNNetDiagonalSSM` (linear recurrence, O(n)) and causal `AddMultiHeadSelfAttention` (O(n²)). The long-range-retrieval accuracy column sorts the arms by reach: TokenShift stuck at chance, DiagonalSSM above chance, conv and attention both solving fully — the cost/reach trade made concrete. Pure CPU, a few seconds.
