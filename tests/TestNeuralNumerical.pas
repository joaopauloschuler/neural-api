...
begin
  ...
  // Fix the RNG before building the net so the random weight init (and hence
  // this finite-difference check) is deterministic and independent of how many
  // random draws earlier tests in the suite made.
  RandSeed := 424242;
  NN := TNNet.Create();
  Input := TNNetVolume.Create(3, 1, 2);
  InputPlus := TNNetVolume.Create(3, 1, 2);
...
procedure TTestNeuralNumerical.TestMaxBlurPoolGradientCheck;
begin
  // TNNetMaxBlurPool: anti-aliased maxpool (dense stride-1 max -> fixed
  // binomial blur -> subsample by stride). The blur weights are constant (no
  // gradient flows to them); Backpropagate must still route the input gradient
  // correctly through both the fixed blur (a transposed conv) and the max
  // selection (each blur tap routed to its dense-max argmax cell). The
  // Sin-seeded inputs give strictly distinct values on this tiny 4x4x2 shape so
  // every dense-max window has an unambiguous argmax and central differences
  // match the analytic input gradient.
  RandSeed := 424242;
  LayerInputGradientCheck(Self, TNNetMaxBlurPool.Create(2),
    'MaxBlurPool', 4, 4, 2, 0.01);
end;

procedure TTestNeuralNumerical.TestMaxBlurPoolLoadFromString;
var
  NN, NN2: TNNet;
  Input: TNNetVolume;
  Saved, Saved2: string;
  i: integer;
begin
  // Round-trip a net containing TNNetMaxBlurPool. SaveToString ->
  // LoadFromString -> SaveToString must be byte-identical, proving the integer
  // pool params (FStruct[0..2]) survive both dispatch points, and the reloaded
  // layer must reproduce the same output.
  RandSeed := 424242;
  NN := TNNet.Create();
  Input := TNNetVolume.Create(4, 4, 2);
  try
    NN.AddLayer(TNNetInput.Create(4, 4, 2, 1));
    NN.AddLayer(TNNetMaxBlurPool.Create(2));
    ...
    try
      NN2.LoadFromString(Saved);
      AssertTrue('MaxBlurPool round-trip class identity',
        NN2.GetLastLayer is TNNetMaxBlurPool);
      Saved2 := NN2.SaveToString();
      AssertEquals('MaxBlurPool SaveToString round-trip equality',
        Saved, Saved2);

      NN2.Compute(Input);
      for i := 0 to NN.GetLastLayer.Output.Size - 1 do
        AssertEquals('MaxBlurPool round-trip output at ' + IntToStr(i),
          NN.GetLastLayer.Output.Raw[i], NN2.GetLastLayer.Output.Raw[i], 1e-6);
    finally
      NN2.Free;
    end;
...
procedure TTestNeuralNumerical.TestMaxBlurPoolShiftInvariance;
var
  NNMax, NNBlur: TNNet;
  Base, Shifted: TNNetVolume;
  x, y, d, s: integer;
  MaxChange, BlurChange, Diff: TNeuralFloat;
  cShifts: integer;
...
begin
  ...
  // MaxBlurPool output LESS (on average) than a plain strided MaxPool output,
  // which is the whole point of anti-aliased pooling (Zhang 2019). We average
  // the mean output change over several 1px horizontal shifts of a smooth
  // 8x8x2 input. Square input keeps both pools on their fast paths.
  RandSeed := 424242;
  ...
  try
    ...
    NNMax.AddLayer(TNNetInput.Create(8, 8, 2, 1));
    NNMax.AddLayer(TNNetMaxPool.Create(2));
    NNBlur.AddLayer(TNNetInput.Create(8, 8, 2, 1));
    NNBlur.AddLayer(TNNetMaxBlurPool.Create(2));
    ...
    begin
      ...
      MaxChange := MaxChange + MeanOutputChange(NNMax);
      BlurChange := BlurChange + MeanOutputChange(NNBlur);
      Inc(cShifts);
    end;
    MaxChange := MaxChange / cShifts;
    BlurChange := BlurChange / cShifts;
    Diff := MaxChange - BlurChange;

    AssertTrue('MaxBlurPool should change less under 1px shift than MaxPool' +
      ' (maxChange=' + FloatToStr(MaxChange) +
      ' blurChange=' + FloatToStr(BlurChange) + ')',
      BlurChange < MaxChange);
...
procedure TTestNeuralNumerical.TestMultiHeadSDPAConcatGradientCheck;
// MHA-b: numerical-gradient check on the input gradient flowing through
// per-head SDPA + TNNetDeepConcat. H=2, d_k=4, SeqLen=3.
var
  NN: TNNet;
  Input, InputPlus, Desired: TNNetVolume;
...
begin
  ...
  try
    ...
    begin
      ...
      if Abs(numericalGrad - analyticalGrad) > maxErr then
        maxErr := Abs(numericalGrad - analyticalGrad);
      AssertTrue('MHA concat input gradient at ' + IntToStr(i) +
        ' num=' + FloatToStr(numericalGrad) + ' ana=' + FloatToStr(analyticalGrad),
        Abs(numericalGrad - analyticalGrad) < 0.01);
    end;
  finally
...
procedure TTestNeuralNumerical.TestTransformerEncoderBlockGradientCheck;
// Numerical-gradient check on the input gradient flowing through the full
// AddTransformerEncoderBlock builder (LayerNorm -> MHA -> residual ->
// LayerNorm -> SwiGLU FFN -> residual), all token-wise. Verified for BOTH
// PreNorm=True and PreNorm=False. Tiny shape: d_model=8, heads=2, seq=3,
// d_ff=16.  Input is (SeqLen,1,d_model); output must stay (SeqLen,1,d_model).
var
  d_model, Heads, SeqLen, d_ff: integer;
  ...
  begin
    ...
    try
      ...
      // Output shape must match input shape so blocks can be stacked.
      AssertTrue('TEB output SizeX', NN.GetLastLayer.Output.SizeX = SeqLen);
      AssertTrue('TEB output SizeY', NN.GetLastLayer.Output.SizeY = 1);
      AssertTrue('TEB output Depth', NN.GetLastLayer.Output.Depth = d_model);
      ...
      NN.Compute(Input);
      for i := 0 to NN.GetLastLayer.Output.Size - 1 do
        AssertTrue('TEB finite forward at ' + IntToStr(i),
      ...
      begin
        ...
        if Abs(numericalGrad - analyticalGrad) > maxErr then
          maxErr := Abs(numericalGrad - analyticalGrad);
        AssertTrue('TEB PreNorm=' + BoolToStr(PreNorm, true) +
...
// MHA-c: numerical-gradient check on the input gradient flowing through the
// full AddMultiHeadSelfAttention builder (split -> SDPA -> concat ->
// token-wise PointwiseConvLinear out-projection). d_model=8, heads=2, seq=3.
var
  NN: TNNet;
  Input, InputPlus, Desired: TNNetVolume;
  d_model, Heads, SeqLen, i: integer;
...
procedure TTestNeuralNumerical.TestWeightNormLinearForward;
var
  NN: TNNet;
  Input: TNNetVolume;
  WN: TNNetWeightNormLinear;
  o, i: integer;
  Norm, Expected, HatNorm: TNeuralFloat;
  N: integer;
begin
  // After normalization each output neuron's effective (unit-norm) weight
  // vector must have ~unit L2 norm over its inputs, and the output must equal
  // bias + dot(w_hat, x) computed independently.
  NN := TNNet.Create();
  Input := TNNetVolume.Create(1, 1, 6);
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 6, 1));
    WN := TNNetWeightNormLinear.Create(4);
    NN.AddLayer(WN);

    for i := 0 to Input.Size - 1 do
      Input.Raw[i] := Sin(i * 0.5) * 0.5 + 0.2;

    // Fill the raw weights with a non-trivial, non-unit-norm pattern so the
    // check is meaningful.
    for o := 0 to WN.Neurons.Count - 1 do
      for i := 0 to WN.Neurons[o].Weights.Size - 1 do
        WN.Neurons[o].Weights.Raw[i] := (o + 1) * 1.5 + i * 0.7 - 2.0;

    NN.Compute(Input);

    N := WN.Neurons[0].Weights.Size;
    for o := 0 to WN.Neurons.Count - 1 do
    begin
      // Reconstruct the normalized weights the same way Compute does, then
      // measure their L2 norm directly and the dot-product output.
      Norm := 0;
      for i := 0 to N - 1 do
        Norm := Norm + Sqr(WN.Neurons[o].Weights.Raw[i]);
      Norm := Sqrt(Norm + 1e-5);

      // L2 norm of w_hat = w/Norm must be ~1 (up to eps).
      HatNorm := 0;
      for i := 0 to N - 1 do
        HatNorm := HatNorm + Sqr(WN.Neurons[o].Weights.Raw[i] / Norm);
      HatNorm := Sqrt(HatNorm);
      AssertEquals('WeightNormLinear w_hat L2 norm neuron ' + IntToStr(o),
        1.0, HatNorm, 1e-4);

      // Output must equal bias + dot(w_hat, x).
      Expected := WN.Neurons[o].Bias;
      for i := 0 to N - 1 do
        Expected := Expected +
          (WN.Neurons[o].Weights.Raw[i] / Norm) * Input.Raw[i];
      AssertEquals('WeightNormLinear output neuron ' + IntToStr(o),
        Expected, NN.GetLastLayer.Output.Raw[o], 1e-5);
    end;
  finally
    NN.Free;
    Input.Free;
...
procedure TTestNeuralNumerical.TestWeightNormLinearGradientCheck;
var
  NN: TNNet;
  Input, InputPlus, Desired: TNNetVolume;
  WN: TNNetWeightNormLinear;
...
begin
  ...
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 5, 1)); // pError=1 resizes error volumes
    WN := TNNetWeightNormLinear.Create(3);
    NN.AddLayer(WN);
    // Learning rate 1, batch update on: deltas accumulate -1*gradient and
    // weights are NOT modified, so finite-difference checks stay valid.
    NN.SetLearningRate(1.0, 0.0);
    ...
    // Non-trivial raw weights (biases keep their default initialization).
    // A non-tiny per-neuron norm keeps the normalization scale (1/norm)
    // moderate so the single-precision finite-difference check is accurate.
    for o := 0 to WN.Neurons.Count - 1 do
      for i := 0 to WN.Neurons[o].Weights.Size - 1 do
        WN.Neurons[o].Weights.Raw[i] := 0.5 + o * 0.4 + i * 0.6 - 1.5;
    ...
    begin
      ...
      AssertTrue('WeightNormLinear input gradient check at position ' +
        IntToStr(i) + ' (num=' + FloatToStr(numericalGrad) + ' ana=' +
        FloatToStr(analyticalGrad) + ')',
        Abs(numericalGrad - analyticalGrad) < 0.01);
    end;
    ...
    for o := 0 to WN.Neurons.Count - 1 do
      for i := 0 to WN.Neurons[o].Weights.Size - 1 do
      begin
        WN.Neurons[o].Weights.Raw[i] := WN.Neurons[o].Weights.Raw[i] + epsilon;
        lossPlus := ComputeLoss(Input);
        WN.Neurons[o].Weights.Raw[i] := WN.Neurons[o].Weights.Raw[i] - 2 * epsilon;
        lossMinus := ComputeLoss(Input);
        WN.Neurons[o].Weights.Raw[i] := WN.Neurons[o].Weights.Raw[i] + epsilon;
        numericalGrad := (lossPlus - lossMinus) / (2 * epsilon);

        NN.Compute(Input);
        WN.Neurons[o].ClearDelta;
        NN.Backpropagate(Desired);
        // Backprop accumulates Delta := Delta - LearningRate*gradient.
        // With LearningRate = 1, analytical gradient = -Delta.
        analyticalGrad := -WN.Neurons[o].Delta.Raw[i];

        AssertTrue('WeightNormLinear weight gradient check (' +
          IntToStr(o) + ',' + IntToStr(i) + ') num=' + FloatToStr(numericalGrad) +
          ' ana=' + FloatToStr(analyticalGrad),
          Abs(numericalGrad - analyticalGrad) < 0.01);
      end;
...
procedure TTestNeuralNumerical.TestWeightNormLinearSerializationRoundTrip;
var
  NN, NN2: TNNet;
  WN: TNNetWeightNormLinear;
  Saved, Saved2: string;
...
begin
  ...
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 4, 1));
    WN := TNNetWeightNormLinear.Create({pSize=}3, {pEpsilon=}7.5e-3);
    NN.AddLayer(WN);

    // Perturb the raw weights away from defaults so the saved data is non-trivial.
    for o := 0 to WN.Neurons.Count - 1 do
      for i := 0 to WN.Neurons[o].Weights.Size - 1 do
        WN.Neurons[o].Weights.Raw[i] := 0.13 * (o + 1) - 0.07 * i;
    ...
    try
      NN2.LoadFromString(Saved);
      Saved2 := NN2.SaveToString();
      AssertEquals('WeightNormLinear SaveToString round-trip', Saved, Saved2);
      AssertTrue('WeightNormLinear class name token present in serialized string',
        Pos('TNNetWeightNormLinear', Saved) > 0);
    finally
      NN2.Free;
    end;
  finally
...
procedure TTestNeuralNumerical.TestSpectralNormForward;
var
  NN, NNScaled: TNNet;
  Input: TNNetVolume;
  SN: TNNetSpectralNorm;
  FC: TNNetFullConnectLinear;
  o, i: integer;
  Sigma, ScaledSigma: TNeuralFloat;
begin
  // Spectral-norm forward property: the EFFECTIVE operator W/sigma_1 must have
  // spectral norm ~1. Build a SpectralNorm layer with a pinned rank-structured
  // weight matrix, then mirror the SCALED weights (W/sigma) into a plain
  // FullConnectLinear and assert EstimateSpectralNorm on the scaled matrix ~1.
  NN := TNNet.Create();
  NNScaled := TNNet.Create();
  Input := TNNetVolume.Create(1, 1, 4);
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 4, 1));
    SN := TNNetSpectralNorm.Create({pSizeX=}3, {pSizeY=}1, {pDepth=}1, {pSuppressBias=}1, {pIters=}30);
    NN.AddLayer(SN);

    // A non-degenerate weight matrix (3 neurons x 4 inputs) with a clearly
    // dominant singular direction.
    ...
    AssertTrue('SpectralNorm latent sigma is positive', Sigma > 0.1);

    // Build a FullConnectLinear carrying the SCALED weights W/sigma.
    NNScaled.AddLayer(TNNetInput.Create(1, 1, 4, 1));
    FC := TNNetFullConnectLinear.Create({pSize=}3, {pSuppressBias=}1);
    NNScaled.AddLayer(FC);
    for o := 0 to FC.Neurons.Count - 1 do
      for i := 0 to FC.Neurons[o].Weights.Size - 1 do
        FC.Neurons[o].Weights.Raw[i] := SN.Neurons[o].Weights.Raw[i] / Sigma;

    ScaledSigma := TNNet.EstimateSpectralNorm(FC, 50);
    // Power iteration is approximate; allow a few percent.
    AssertEquals('SpectralNorm effective spectral norm ~ 1', 1.0,
      ScaledSigma, 0.03);
  finally
    NN.Free;
    NNScaled.Free;
...
procedure TTestNeuralNumerical.TestSpectralNormInputGradientCheck;
var
  NN: TNNet;
  Input, InputPlus, Desired: TNNetVolume;
  SN: TNNetSpectralNorm;
...
begin
  // Input numerical-gradient check (central differences, tol 1e-2). sigma is
  // treated as CONSTANT w.r.t. the gradient (the power iteration is not
  // differentiated), so the input gradient is the ordinary linear gradient
  // using the SCALED weights W/sigma. A finite difference of the input never
  // changes sigma (sigma depends only on W), so the numerical derivative
  // matches the analytical scaled-weight input gradient cleanly. The latent
  // WEIGHT gradient is NOT finite-difference-checked here (constant-sigma
  // approximation); the forward property test pins the scaling instead.
  NN := TNNet.Create();
  Input := TNNetVolume.Create(1, 1, 5);
  InputPlus := TNNetVolume.Create(1, 1, 5);
  Desired := TNNetVolume.Create(1, 1, 3);
  ...
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 5, 1));
    SN := TNNetSpectralNorm.Create({pSizeX=}3, {pSizeY=}1, {pDepth=}1, {pSuppressBias=}0, {pIters=}20);
    NN.AddLayer(SN);
    NN.SetLearningRate(1.0, 0.0);
    NN.SetBatchUpdate(true);
    ...
    // keep the single-precision finite difference accurate.
    for o := 0 to SN.Neurons.Count - 1 do
      for i := 0 to SN.Neurons[o].Weights.Size - 1 do
        SN.Neurons[o].Weights.Raw[i] := 0.2 + 0.15 * o - 0.1 * i + 0.05 * o * i;
    ...
    begin
      ...
      AssertTrue('SpectralNorm input gradient check at position ' +
        IntToStr(i) + ' (num=' + FloatToStr(numericalGrad) + ' ana=' +
        FloatToStr(analyticalGrad) + ')',
        Abs(numericalGrad - analyticalGrad) < 0.01);
    end;
...
procedure TTestNeuralNumerical.TestSpectralNormSerializationRoundTrip;
var
  NN, NN2: TNNet;
  SN: TNNetSpectralNorm;
  Input: TNNetVolume;
  ...
  o, i: integer;
begin
  // SpectralNorm in the MIDDLE of a net: save/load and assert structure string
  // and Compute match (the Iters count round-trips via FStruct[5]).
  NN := TNNet.Create();
  Input := TNNetVolume.Create(1, 1, 4);
  try
    NN.AddLayer(TNNetInput.Create(1, 1, 4, 1));
    SN := TNNetSpectralNorm.Create({pSizeX=}5, {pSizeY=}1, {pDepth=}1, {pSuppressBias=}0, {pIters=}7);
    NN.AddLayer(SN);
    ...
    for o := 0 to SN.Neurons.Count - 1 do
      for i := 0 to SN.Neurons[o].Weights.Size - 1 do
        SN.Neurons[o].Weights.Raw[i] := 0.13 * (o + 1) - 0.07 * i;
    ...
    try
      NN2.LoadFromString(Saved);
      NN2.Compute(Input);
      AssertTrue('SpectralNorm token present in serialized string',
        Pos('TNNetSpectralNorm', Saved) > 0);
      AssertEquals('SpectralNorm structure string matches after round-trip',
        NN.Layers[1].SaveStructureToString(),
        NN2.Layers[1].SaveStructureToString());
      for i := 0 to NN.GetLastLayer.Output.Size - 1 do
