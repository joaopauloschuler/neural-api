...
(All four LANDED 2026-06-28 — removed:
- AVX-vectorized `TNNetGlobalSumPool.Compute` (pixel-outer contiguous-span
  reduction), commit 30a3b27b;
- OpenCL forward offload for `TNNetGramMatrix.Compute` via the `cai_dot_product`
  kernel + `GramMatrixOpenCLParity` test (PoCL max|diff|=2e-9), commit 319fce71;
- AVX-vectorized forward of `TNNetAdaptiveAvgPool` (span-scale) and
  `TNNetAdaptiveMaxPool` (Move-seed + AVXMax, argmax backward untouched),
  commit 0e2ed264;
