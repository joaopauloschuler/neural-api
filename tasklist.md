...
- [X] TNNetHouseholderLinear — an EXACTLY-orthogonal dense layer whose `n×n`
      ...
      neurons of `Depth` weights, `n = Depth`); the forward applies the
      reflections to the input one at a time (`O(K·n)` per sample, never
      materializing `Q`), `y = Q·x (+ bias)`. Because every `H_i` is its own
      inverse and orthogonal, `Q` is orthogonal for ANY values of `v_i` — so the
      map is norm/volume preserving (`‖y‖ = ‖x‖` when bias off) WITHOUT any
      constrained optimization or re-projection, and the inverse is just the
      reflections applied in reverse order (`Qᵀ = H_K·…·H_1`). This fills a real
      gap and is structurally DISTINCT from everything already here:
      `TNNetSpectralNorm` only divides by the largest singular value (bounds
      `σ_1 ≈ 1` but leaves the other singular values free — NOT orthogonal);
      the structured-matrix layers `TNNetCirculantLinear`/`TNNetToeplitz*`/
      `TNNetSpectralNorm` constrain the matrix FORM, not its orthogonality; and
      the Muon work (orthogonalized gradient UPDATE, tasklist ~L561) orthogonalizes
      the step, not the weight. Why it matters: exact orthogonality kills
      exploding/vanishing gradients in deep stacks (the Jacobian is an isometry),
      and an invertible norm-preserving block is the building block for
      normalizing flows / reversible nets. Backward differentiates through the
      reflection product: caching each intermediate `x_i = H_i·…·H_K·x` lets the
      per-`v_i` gradient be computed in closed form (`dH_i/dv_i` is the standard
      Householder derivative; the per-reflection cost stays `O(n)`), propagating
      `dL/dx = Qᵀ·dL/dy` back to the input. Ship the usual trio — declaration
      with the `// Coded by Claude (AI).` comment, dispatch-table wiring,
      numerical-gradient test (input grad + all `K` reflection-vector grads,
      reseed `RandSeed := 424242`) PLUS an explicit orthogonality assertion that
      `‖Q·x‖ ≈ ‖x‖` and `Qᵀ·Q·x ≈ x` to <1e-5 — plus a `TNNet.AddHouseholderLinear(N, NumReflections, UseBias)` composite builder (default `K = n` for a full-rank orthogonal group element; `K < n` gives a cheaper sub-group). Capstone `examples/OrthogonalRNN/` (or `examples/HouseholderOrthogonal/`): a deep plain stack of these layers on a long-range synthetic task (e.g. the copy/adding problem) showing the gradient norm stays ~constant with depth where an equivalent unconstrained `TNNetFullConnectLinear` stack blows up or collapses, with a short note on the `K` vs cost/expressivity trade-off. Edge cases: guard the `v_iᵀ·v_i → 0` denominator (degenerate reflection ≈ identity) and verify serialization round-trips the reflection vectors.
