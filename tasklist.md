...
- [X] Proper Unicode NFC normalization in `neuralhftokenizer.pas`
      (`AddNormalizer`). Real CANONICAL composition/decomposition is now done:
      `UnicodeNormalize` applies full NFD (recursive canonical decomposition +
      combining-class canonical ordering) and NFC (NFD then canonical
      composition), driven by compact tables in `neural/neuralnfctables.inc`
      (auto-generated by `tools/gen_nfc_tables.py` from Python `unicodedata`)
      plus algorithmic Hangul L/V/T. Wired into the encode path before
      pre-tokenization; parity tests pin precomposed<->decomposed UTF-8 bytes
      and token-id folding (`TestNFCComposesDecomposedInput`,
      `TestNFDDecomposesPrecomposedInput`, `TestNFCNormalizerFoldsTokenIds`).
