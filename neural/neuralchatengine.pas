...
begin
  ...
  begin
    ...
    // GenerateTokensMTPSpeculative prefills through the width-2 trunk itself,
    // so the --prefill-window twins would be built and never fed.
    if (TrunkWidth = 2) and (Opt.PrefillWindow > 0) then
    begin
      Notice(Format('[--prefill-window %d ignored: --mtp prefills through its' +
        ' own width-2 trunk]', [Opt.PrefillWindow]));
      Opt.PrefillWindow := 0;
      Opt.PrefillTailWindow := 1;
    end;
  end;
  ...
  begin
    ...
    try
      MTPNet := BuildQwen35MTPFromSafeTensors(MTPWeightsPath, MTPConfig,
        {pSeqLen=}1, {pTrainable=}false, {pQuantizeInt8=}Opt.WeightMode <> cwmFP32);
    except
      on E: Exception do
      begin
        FreeAndNil(MTPNet);
  ...
  // CachedTokens from claiming a reuse that never happens, and LoadModel
  // from sizing a checkpoint store nobody reads.
  if MTPActive then
  begin
    ReuseOK := false;
    StateReuseOK := false;
