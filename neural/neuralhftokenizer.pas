...
//     for WORD/CHAR).
//
// Unicode \p{L} / \p{N} / \p{M} classification for the pre-tokenization
// regexes uses EXACT general-category range tables (Unicode 15.0,
// neuralunicodeclasses.inc, auto-generated by
// tools/gen_unicode_class_tables.py) with a binary search per non-ASCII
// codepoint -- validated by real-vocab fuzzing against HF `tokenizers`
// on the Qwen3.6-27B tokenizer.
//
...
var
  ...
  Offset, Count, K, CountM1: integer;
begin
...
// Exact Unicode general-category range tables for \p{L} / \p{N} / \p{M}
// (neuralunicodeclasses.inc, auto-generated by
// tools/gen_unicode_class_tables.py). These replaced the historical
// hand-written "major scripts" approximation: real-vocab fuzzing against
// the Qwen3.6-27B tokenizer showed the approximation splits differently
// from HF `tokenizers` whenever an uncovered script character (Bengali
// letters, Gujarati digits, ...) borders a covered class -- the Qwen3.5
// pattern's [^\r\n\p{L}\p{N}]?[\p{L}\p{M}]+ alternative is especially
// sensitive.
{$include neuralunicodeclasses.inc}

// Binary search over an ascending inclusive-[lo,hi] range table.
function CPInRanges(CP: cardinal;
  const Ranges: array of TUnicodeRangePair): boolean;
var
  Lo, Hi, Mid: integer;
begin
  Lo := 0;
  ...
  begin
    Mid := (Lo + Hi) shr 1;
    if CP < Ranges[Mid][0] then Hi := Mid - 1
    else if CP > Ranges[Mid][1] then Lo := Mid + 1
    else Exit(true);
  end;
  Result := false;
end;

// \p{L} -- exact Unicode category L* (ASCII fast path first).
function IsLetterCP(CP: cardinal): boolean;
begin
  if CP < 128 then
    Exit(((CP >= Ord('A')) and (CP <= Ord('Z'))) or
      ((CP >= Ord('a')) and (CP <= Ord('z'))));
  Result := CPInRanges(CP, cUnicodeLetterRanges);
end;

// \p{N} -- exact Unicode category N* (ASCII fast path first).
function IsNumberCP(CP: cardinal): boolean;
begin
  if CP < 128 then
    Exit((CP >= Ord('0')) and (CP <= Ord('9')));
  Result := CPInRanges(CP, cUnicodeNumberRanges);
end;
...
// \p{M} -- exact Unicode category M* (Mn, Mc, Me), used by the Qwen3.5
// marks-join-letters Split mode.
function IsMarkCP(CP: cardinal): boolean;
begin
  if CP < 128 then Exit(false);
  Result := CPInRanges(CP, cUnicodeMarkRanges);
end;
...
var
  B, Shifted, CPToByteHi: integer;
  CP: cardinal;
begin
  CPToByteHi := High(FCPToByte);
  for B := 0 to CPToByteHi do FCPToByte[B] := -1;
  Shifted := 0;
  for B := 0 to 255 do
  begin
...
var
  ...
  Model: string;
  TokCount, Cnt, TokCountM1: integer;
  ScoreCount, TypeCount, MergeCount, MergeCountM1: Int64;
...
var
  ...
  TokCountM1, MergesNumM1, MergesHi, AddedHi: integer;
...
begin
  ...
  begin
    ...
    MergesNumM1 := MergesNum - 1;
    for Cnt := 0 to MergesNumM1 do
    begin
    ...
    MergesHi := High(Merges);
    for Cnt := 0 to MergesHi do Merges[Cnt] := '';
    for Mi := 0 to MergesNumM1 do
    begin
      Combined := FMerges[Mi];
...
begin
  ...
  SetLength(CPByte, Length(Segment) + 1);
  Total := 0;
  Position := 1;
...
var
  ...
  CPStr: array of string;
  CPByte: array of integer; // 1-based byte offset where each char starts (+sentinel)
  Total, Position, Idx, RunStart, RunEnd, LastNL, DigitCnt, DigitsMax: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      Sub := Copy(PieceText, CPStart[I], CPJ - CPStart[I]);
...
begin
  ...
  SetLength(CPByte, Length(WordText) + 1);
  Total := 0;
  Position := 1;
  ...
  try
    ...
    begin
      ...
      begin
        Sub := Copy(WordText, CPByte[StartIdx], CPByte[EndIdx] - CPByte[StartIdx]);
...
begin
  ...
  begin
    ...
    NormReplaceHi := High(FNormReplaceFrom);
    for Cnt := 0 to NormReplaceHi do
...
begin
  ...
  try
    ...
    IdsCntM1 := IdsCnt - 1;
    for Cnt := 0 to IdsCntM1 do Result[Cnt] := Ids[Cnt];
  finally
    Ids.Free;
  end;
end;
...
var
  Token: string;
  Position, Cnt, DecReplaceHi, HiNib, LoNib: integer;
  CP: cardinal;
...
begin
  ...
  IdsCntM1 := IdsCnt - 1;
  for Cnt := 0 to IdsCntM1 do Arr[Cnt] := Ids[Cnt];
...
var
  TokenIndex, AddedHi: integer;
begin
  Result := VocabFind(Token);
  AddedHi := High(FAddedTokens);
  if Result < 0 then
    for TokenIndex := 0 to AddedHi do
