...
uses
  ...
  fgl,
  {$ENDIF}
  {$IFDEF THREADSAVERANDOM}RandomEng, {$ENDIF}
...
type
  ...
  TNNetNeuron = class (TMObject)
    ...
    public
      ...
      procedure Assign( neuron : TNNetNeuron );
  ...
  TNNetLayer = class(TMObject)
    ...
    protected
      procedure Assign( layer : TNNetLayer );
    public
      ...
      procedure CalcAdamDelta(); {$IFDEF Release} inline; {$ENDIF}
      procedure UpdateWeightsAdam(); {$IFDEF Release} inline; {$ENDIF}
  ...
  public
    constructor Create; overload; override;
    constructor Create(n: integer); reintroduce; overload;
    destructor Destroy(); override;
  ...
  public
    ...
    constructor Create(pVocabSize, pEmbeddingSize: integer;
      EncodeZero: integer = 0; ScaleEmbedding: TNeuralFloat = 2); reintroduce; overload;
    destructor Destroy; override;
  ...
  TNNetAddNoiseBaseRnd = class(TNNetAddNoiseBase)
  private
    {$IFDEF THREADSAVERANDOM}
    fRandEng : TRandomGenerator;
    {$ENDIF}
  protected
  ...
  // that are zeroed).
  TNNetDropout = class(TNNetAddNoiseBaseRnd)
    protected
      FRate: integer;
  ...
  TNNetRandomMulAdd = class(TNNetAddNoiseBaseRnd)
  protected
  ...
  TNNetChannelRandomMulAdd = class(TNNetAddNoiseBaseRnd)
  protected
  ...
  TNNetMovingScale = class(TNNetIdentityWithoutL2)
    ...
    public
      constructor Create; overload; override;
      constructor Create(pMaxTarget: TNeuralFloat; pChangeRate: TNeuralFloat); reintroduce; overload;
      procedure Compute(); override;
      procedure Backpropagate(); override;
      procedure InitDefault(); override;
  end;
  ...
  private
    ...
    procedure SetPrevLayer(pPrevLayer: TNNetLayer); override;
  public
    constructor Create(A, B: TNNetLayer); reintroduce; overload;
    constructor Create(AIdx, BIdx: integer); reintroduce; overload;
    destructor Destroy(); override;
  ...
  public
    ...
    constructor Create; overload; override;
    constructor Create(SkipBackpropDerivative: integer); reintroduce; overload;
    procedure Compute(); override;
    procedure Backpropagate(); override;
  end;
  ...
  TNNetDepthwiseConvGeLU = class(TNNetDepthwiseConv)
  public
    constructor Create(pMultiplier, pFeatureSize, pInputPadding, pStride: integer); override;
  end;
  ...
  // convolutional layer with GeLU activation function
  TNNetConvolutionGeLU = class(TNNetConvolution)
  public
    constructor Create(pNumFeatures, pFeatureSize, pInputPadding, pStride: integer; pSuppressBias: integer = 0); override;
  end;

  TNNetConvolutionSwish6 = class(TNNetConvolution)
  public
    constructor Create(pNumFeatures, pFeatureSize, pInputPadding, pStride: integer; pSuppressBias: integer = 0); override;
  end;
  ...
  TNNetPoolBase = class(TNNetLayer)
    ...
    protected
      ...
      {$ENDIF}
    public
      constructor Create(pPoolSize: integer; pStride:integer = 0; pPadding: integer = 0); reintroduce; overload; virtual;
      destructor Destroy(); override;
      procedure Backpropagate(); override;
    end;
  ...
  TNNetMaxPoolWithPosition = class(TNNetMaxPool)
    ...
    public
      constructor Create(pPoolSize: integer; pStride: integer; pPadding: integer;
        pLogPosX: integer; pLogPosY: integer; pMaxBackpropX: integer = 0);
      destructor Destroy(); override;
      procedure Backpropagate(); override;
  end;
  ...
  /// neural network
  TTNNetProgress = procedure(Sender : TObject; numCalc, numTotal : integer; var cancel : boolean ) of Object;
  TNNet = class(TMObject)
    ...
    public
      property OnProgress : TTNNetProgress read fProgress write fProgress; // called in Compute!
    public
      constructor Create();
      destructor Destroy(); override;
      ...
      function AddSelfAttentionCAI(Heads: integer): TNNetLayer;
  ...
  {$ENDIF}
    public
      ...
      {$IFDEF OpenCL}
      procedure DisableOpenCL();
      procedure EnableOpenCL(platform_id: cl_platform_id; device_id: cl_device_id; maxNumOpenCLThreads : integer = -1);
      {$ENDIF}
  end;
...
begin
  ...
  FMaxTarget := 1;
  FChangeRate := 1;
  FFloatSt[0] := FMaxTarget;
  FFloatSt[1] := FChangeRate;
end;
...
constructor TNNetPointwiseSoftMax.Create(SkipBackpropDerivative: integer);
begin
  inherited Create();
...
constructor TNNetPointwiseSoftMax.Create;
begin
     inherited;
     // default = 0
     FSkipBackpropDerivative := False;
     FStruct[0] := 0;
end;
...
constructor TNNetAddPositionalEmbedding.Create;
begin
  inherited Create;
  FPositionalEmbedding := TNNetVolume.Create;
  FStruct[0] := 10000
...
constructor TNNetCrop.Create;
begin
  raise Exception.Create('Default constructor not allowed here');
  inherited;
end;
...
{ TNNetDepthwiseConvGeLU }
constructor TNNetDepthwiseConvGeLU.Create(pMultiplier, pFeatureSize,
  pInputPadding, pStride: integer);
begin
  inherited Create(pMultiplier, pFeatureSize, pInputPadding, pStride);
  FActivationFn := @GaussErrorLinUnit;
  FActivationFnDerivative := @GaussErrorLinUnitDerivative;
end;
...
function TNNet.AddSelfAttention(Heads: integer): TNNetLayer;
var
  W : TNNetLayer;
  // Query, Key, ValueT: TNNetLayer; // WT, YT, Value
  PreviousLayer: TNNetLayer;
...
begin
  ...
  AddLayer( TNNetPointwiseConvLinear.Create(EmbeddingDim) );
  if HasNorm then AddLayer( TNNetMovingScale.Create() );
  AddLayer( TNNetSum.Create([ GetLastLayer(), AttendedPlusPrev]) );
  if HasNorm
  then Result := AddLayer( TNNetMovingScale.Create() )
  else Result := GetLastLayer();
...
begin
  PrevLayer := GetLastLayer();
  EmbeddingDim := PrevLayer.Output.Depth;
  Attended := AddSelfAttentionCAI(Heads);
...
procedure TNNetDataParallelism.EnableOpenCL(platform_id: cl_platform_id;
  device_id: cl_device_id; maxNumOpenCLThreads : integer = -1);
var
  I: integer;
begin
  if Count > 0 then
...
{ TNNetConvolutionGeLU }

constructor TNNetConvolutionGeLU.Create(pNumFeatures, pFeatureSize,
  pInputPadding, pStride: integer; pSuppressBias: integer = 0);
begin
  inherited Create(pNumFeatures, pFeatureSize, pInputPadding, pStride, pSuppressBias);
  FActivationFn := @GaussErrorLinUnit;
...
{ TNNetConvolutionSwish6 }

constructor TNNetConvolutionSwish6.Create(pNumFeatures, pFeatureSize,
  pInputPadding, pStride, pSuppressBias: integer);
begin
  inherited Create(pNumFeatures, pFeatureSize, pInputPadding, pStride, pSuppressBias);
  FActivationFn := @Swish6Unit;
  FActivationFnDerivative := @Swish6Derivative;
end;
...
begin
  ...
  {$IFDEF THREADSAVERANDOM}
  fRandEng := TRandomGenerator.Create(raChaCha);
  fRandEng.Init(0);
  {$ENDIF}
...
{$IFDEF THREADSAVERANDOM}
function TNNetPoolBase.Random(range : integer) : integer;
begin
     Result := fRandEng.RandInt(range);
end;
{$ENDIF}
...
//  PrevNumElements, PrevMissedElements: integer;
  {$IFDEF AVX64}PtrNeuronDelta, {$ENDIF} PtrPreparedInput: TNeuralFloatArrPtr;
  PrevPtrA, PrevPtrB: TNeuralFloatArrPtr;
//  NeuronWeights: integer;
//  LocalLearningErrorDerivPtr: pointer;
//  localNumElements : Integer;
...
var
  ...
  //{SrcPtr,} LocalDestPtr: TNeuralFloatArrPtr;
  //SmoothLocalOutputErrorDerivPtr: pointer;
  {$IFDEF Debug} {$IFDEF AVX64} PrevNumElements, PrevMissedElements: integer;  {$ENDIF} {$ENDIF}
  {$IFDEF Release}{$IFDEF AVX64} PtrNeuronDelta : TNeuralFloatArrPtr; {$ENDIF} {$ENDIF}

  PtrPreparedInput: TNeuralFloatArrPtr;
  PrevPtrA, PrevPtrB: TNeuralFloatArrPtr;

  //LocalLearningErrorDerivPtr: pointer;
  {$IFDEF Debug} {$IFDEF AVX64}NeuronWeights, localNumElements, MissedElements: integer;  {$ENDIF} {$ENDIF}
  MaxPrevX, MaxPrevY: integer;
begin
  MaxX := OutputError.SizeX - 1;
  MaxY := OutputError.SizeY - 1;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          OutputRawPos := FOutputErrorDeriv.GetRawPos(OutputX, OutputY);
          //TODO: the next line is probably wrong.
          // this is actually never used afterwards -> I comment it out
          //if (FCalculatePrevLayerError) then LocalDestPtr := LocalPrevError.GetRawPtr(OutputX, OutputY);
          PtrPreparedInput := FInputPrepared.GetRawPtr(OutputX, OutputY);
          CanBackpropOnPos :=
            //(PrevX >= 0) and (PrevY >= 0) and
...
begin
  ...
  begin
    RefreshNeuronWeightList();
    AfterWeightUpdate();
    if FDotCL.PrepareForCompute(FConcatedWInter, FPrevLayer.FOutput, FVectorSize) <> CL_SUCCESS then
       FreeAndNil(fDotCL);
  end;
end;
{$ENDIF}
...
constructor TNNetEmbedding.Create;
begin
  inherited;
  ...
  FEncodeZero := False;
  FScaleEmbedding := 2;
  FStruct[0] := FVocabSize;
  FStruct[1] := FEmbeddingSize;
  FStruct[2] := 0;
  FFloatSt[0] := 2;
  if FNeurons.Count < 1 then AddMissingNeurons(1);
  SetNumWeightsForAllNeurons(FVocabSize, 1, FEmbeddingSize);
  InitDefault();
  AfterWeightUpdate();
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if S2.Count > 0 then
      begin
        for I := 0 to Min(S2.Count - 1, High(St)) do Ft[I] := StrToFloat(S2[I], fmt);
      end;
    end
    else
    ...
    if S[0] = 'TNNetPointwiseSoftMax' then Result := TNNetPointwiseSoftMax.Create(St[0]) else
    ...
    if S[0] = 'TNNetConvolutionGeLU' then Result := TNNetConvolutionGeLU.Create(St[0], St[1], St[2], St[3], St[4]) else
    ...
    if S[0] = 'TNNetDepthwiseConvGeLU' then Result := TNNetDepthwiseConvGeLU.Create(St[0], St[1], St[2], St[3]) else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
           fProgress( self, idxInput, maxIdxInput + 1, cancel);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        {$IFNDEF Debug}
        FLayers[LayerCnt].ForceMaxAbsoluteDelta(vMax);
        {$ENDIF}
        {$IFDEF Debug}
...
begin
  ...
  FDotProductKernel.MessageProc := Self.MessageProc;
  FDotProductKernel.ErrorProc := self.ErrorProc;
  FDotProductKernel.Prepare;
  for LayerCnt := 0 to GetLastLayerIdx() do
  begin
...
procedure TNNetLayer.Assign(layer: TNNetLayer);
var i : integer;
begin
     // ###########################################
     // #### Assign all elements common with the layer object
     //FStruct := layer.FStruct;
//     fOutput.Copy(layer.FOutput);
//     FOutputRaw.Copy(layer.FOutputRaw);
//     FOutputError.Copy(layer.FOutputError);
//     FOutputErrorDeriv.Copy(FOutputErrorDeriv);
//     FSuppressBias := layer.FSuppressBias;
//
//     FNeurons := TNNetNeuronList.Create();
//     for i := 0 to layer.FNeurons.Count - 1 do
//         fNeurons.Add(layer.FNeurons[i].Clone);
//
//     AfterWeightUpdate;


       assert( layer.FNeurons.Count = FNeurons.Count, 'neuron count does not check');
       for i := 0 to layer.FNeurons.Count - 1 do
           TNNetNeuron(fNeurons[i]).Assign( layer.FNeurons[i] );


     //FLinkedNeurons := layer.FLinkedNeurons;
//     FActivationFn := layer.FActivationFn;
//     FActivationFnDerivative := layer.FActivationFnDerivative;
//     FLearningRate := layer.FLearningRate;
//     FL2Decay := layer.FL2Decay;
//     //FPrevLayer := nil;
//     FInertia := layer.FInertia;
//     FBatchUpdate := layer.FBatchUpdate;
//     FSmoothErrorPropagation := layer.FSmoothErrorPropagation;
//     FDepartingBranchesCnt := layer.FDepartingBranchesCnt;
//     FBackPropCallCurrentCnt := layer.FBackPropCallCurrentCnt;
//     FBackwardTime := 0;
//     FForwardTime := 0;
end;
...
begin
  ...
  begin
    if I > 0 then Result := Result + ';';
    Result := Result + FloatToStr(FFloatSt[I], fmt);
  end;
end;
...
procedure TNNetNeuron.Assign(neuron: TNNetNeuron);
begin
...
{ TNNetAddNoiseBaseRnd }

constructor TNNetAddNoiseBaseRnd.Create;
begin
     inherited;
...
destructor TNNetAddNoiseBaseRnd.Destroy;
begin
     {$IFDEF THREADSAVERANDOM}
     fRandEng.Free;
     {$ENDIF}
...
function TNNetAddNoiseBaseRnd.Random(range: integer): integer;
begin
     {$IFDEF THREADSAVERANDOM}
     Result := fRandEng.RandInt(range);
     {$ELSE}
