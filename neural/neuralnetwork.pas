...
type
  ...
  // Log(x) = ln(max(x, 1e-8)). The 1e-8 floor guards against ln(0)/ln(neg).
  ...
  // Threshold(x) = x if x > theta else value. Generalizes ReLU
  // (theta=0, value=0). Derivative is 1 for x > theta, else 0 (an
  ...
  public
    constructor Create(); overload;
    constructor Create(pTheta, pValue: TNeuralFloat); overload;
    procedure Compute(); override;
  end;
  ...
  /// Huber loss output layer.
  // Forward is an identity passthrough so users can call Net.Compute for
  // inference. Training relies on the framework seeding the last layer's
  // FOutputError with (output - target); this layer's Backpropagate clips
  // each element of FOutputError to [-delta, +delta] before propagating to
  // the previous layer. This corresponds to the gradient of the piecewise
  // Huber loss:
  //   L = 0.5 * (y-t)^2                  if |y-t| <= delta
  //   L = delta * (|y-t| - 0.5*delta)    if |y-t|  > delta
  ...
  public
    constructor Create(); overload; override;
    constructor Create(pDelta: TNeuralFloat); overload;
    procedure Backpropagate(); override;
  end;
  ...
  /// This layer does a proper per-sample layer normalization. Each input
  ...
  /// "Attention is off by one" softmax (Miller, 2023):
...
begin
  ...
  begin
    ...
    begin
      ...
      if PrevValue > 0 then
      begin
        FOutput.FData[OutputCnt] := PrevValue * PrevValue;
        FOutputErrorDeriv.FData[OutputCnt] := 2 * PrevValue;
      end
      else
      begin
        FOutput.FData[OutputCnt] := 0;
  ...
  begin
    ...
    begin
      PrevValue := LocalPrevOutput.FData[OutputCnt];
      if PrevValue > 0
        then FOutput.FData[OutputCnt] := PrevValue * PrevValue
...
procedure TNNetGLU.Backpropagate();
var
  StartTime: double;
  MaxX, MaxY, MaxD: integer;
  X, Y, D, HalfDepth: integer;
...
constructor TNNetHuberLoss.Create();
begin
  Create(1.0);
end;

constructor TNNetHuberLoss.Create(pDelta: TNeuralFloat);
begin
  inherited Create();
  if pDelta <= 0 then
    FErrorProc('TNNetHuberLoss delta must be > 0.');
  FFloatSt[0] := pDelta;
end;

procedure TNNetHuberLoss.Backpropagate();
var
  StartTime: double;
  Idx, SizeM1: integer;
  Delta, V: TNeuralFloat;
begin
  StartTime := Now();
  Inc(FBackPropCallCurrentCnt);
  if FBackPropCallCurrentCnt < FDepartingBranchesCnt then exit;
  ...
  begin
    ...
    if V >  Delta then V :=  Delta
    else if V < -Delta then V := -Delta;
    FOutputError.FData[Idx] := V;
  end;
  FBackwardTime := FBackwardTime + (Now() - StartTime);
  inherited BackpropagateNoTest();
end;
...
procedure TNNetLog.Compute();
const
  LOG_EPS: TNeuralFloat = 1e-8;
var
  SizeM1: integer;
  LocalPrevOutput: TNNetVolume;
  OutputCnt: integer;
...
begin
  ...
  // Log(x) = ln(max(x, 1e-8)); derivative is 1/max(x, 1e-8).
  if (FOutput.Size = FOutputError.Size) and (FOutputErrorDeriv.Size = FOutput.Size) then
  begin
    for OutputCnt := 0 to SizeM1 do
    begin
...
constructor TNNetThreshold.Create();
begin
  Create(0, 0);
end;

constructor TNNetThreshold.Create(pTheta, pValue: TNeuralFloat);
begin
  inherited Create();
  FFloatSt[0] := pTheta;
  FFloatSt[1] := pValue;
...
begin
  ...
  for CntD := 0 to MaxD do
    for CntY := 0 to MaxY do
      for CntX := 0 to MaxX do
        FOutput[CntX, CntY, CntD] :=
          FPrevLayer.FOutput[MaxX - CntX, CntY, CntD];
  FForwardTime := FForwardTime + (Now() - StartTime);
end;
  ...
  begin
    ...
    for CntD := 0 to MaxD do
      for CntY := 0 to MaxY do
        for CntX := 0 to MaxX do
          FPrevLayer.OutputError.Add(CntX, CntY, CntD,
            FOutputError[MaxX - CntX, CntY, CntD]);
  end;
  LocalNow := Now();
  FBackwardTime := FBackwardTime + (LocalNow - StartTime);
  if Assigned(FPrevLayer) then FPrevLayer.Backpropagate();
...
begin
  ...
  for CntD := 0 to MaxD do
    for CntY := 0 to MaxY do
      for CntX := 0 to MaxX do
        FOutput[CntX, CntY, CntD] :=
          FPrevLayer.FOutput[CntX, MaxY - CntY, CntD];
  FForwardTime := FForwardTime + (Now() - StartTime);
end;
...
{ TNNetPReLU }
constructor TNNetPReLU.Create();
begin
  // He et al. 2015 initial slope.
  Create(0.25);
end;

constructor TNNetPReLU.Create(pInitialAlpha: TNeuralFloat);
begin
  inherited Create();
  FInitialAlpha := pInitialAlpha;
  FFloatSt[0] := pInitialAlpha;
...
procedure TNNetPReLU.SetPrevLayer(pPrevLayer: TNNetLayer);
begin
  inherited SetPrevLayer(pPrevLayer);
  // Override the base which allocates Depth weights: PReLU (shared) has
  // exactly one learnable scalar.
  SetNumWeightsForAllNeurons(1, 1, 1);
  InitDefault();
end;

procedure TNNetPReLU.Compute();
var
  StartTime: double;
  alpha, x: TNeuralFloat;
  i, SizeM1: integer;
  LocalPrevOutput: TNNetVolume;
  HasDeriv: boolean;
begin
  ...
  if FNeurons[0].FWeights.Size <> 1 then
  begin
    FErrorProc('Neuron weight count must be 1 at TNNetPReLU.');
  end;
  {$ENDIF}
  alpha := FNeurons[0].FWeights.Raw[0];
  LocalPrevOutput := FPrevLayer.Output;
  ...
  begin
    ...
    begin
      ...
      begin
        FOutput.FData[i] := x;
        FOutputErrorDeriv.FData[i] := 1;
      end
      else
      begin
        FOutput.FData[i] := alpha * x;
        FOutputErrorDeriv.FData[i] := alpha;
      end;
    end;
  end
  else
...
begin
  ...
  begin
    ...
    if x <= 0 then
      gradAlpha := gradAlpha + FOutputError.Raw[i] * x;
  end;
  localNeuron.FDelta.Raw[0] := localNeuron.FDelta.Raw[0] +
    (-FLearningRate) * gradAlpha;
  if (not FBatchUpdate) then
  ...
  begin
    ...
    begin
      ...
      if x > 0 then
        ...
        FPrevLayer.FOutputError.Raw[i] := FPrevLayer.FOutputError.Raw[i] +
          alpha * FOutputError.Raw[i];
    end;
    FPrevLayer.Backpropagate();
  end;
end;

procedure TNNetPReLU.InitDefault();
begin
  if FNeurons.Count < 1 then AddMissingNeurons(1);
  inherited InitDefault();
  FNeurons[0].Weights.Fill(FInitialAlpha);
...
{ TNNetLayerNorm }
constructor TNNetLayerNorm.Create();
begin
  inherited Create();
  FLayerNormEpsilon := 1e-5;
  FInvStdDev := 1;
  FNormalized := TNNetVolume.Create();
end;

destructor TNNetLayerNorm.Destroy();
begin
  FNormalized.Free;
  inherited Destroy();
end;
...
procedure TNNetLayerNorm.Compute();
var
  StartTime: double;
...
procedure TNNetLayerNorm.Backpropagate();
var
  StartTime: double;
...
begin
  ...
  SizeM1 := FOutput.Size - 1;
  // y_i = exp(x_i) / (1 + sum_j exp(x_j))
  // Numerically stable with the standard "shift by max" trick:
  //   m = max_i x_i
  //   s = exp(-m) + sum_j exp(x_j - m)   ; the "1" becomes exp(-m) after shift
  //   y_i = exp(x_i - m) / s
  if SizeM1 < 0 then
  begin
    FForwardTime := FForwardTime + (Now() - StartTime);
  ...
  begin
    for i := 0 to SizeM1 do
      FOutput.FData[i] := FOutput.FData[i] / S;
  end;
  FForwardTime := FForwardTime + (Now() - StartTime);
end;

procedure TNNetSoftMaxOne.Backpropagate;
var
  StartTime: double;
  Dot, Yi: TNeuralFloat;
  i, SizeM1: integer;
begin
  ...
  begin
    ...
    begin
      // Softmax-one Jacobian: the "+1" in the denominator has zero partial
      // derivative w.r.t. every x, so d y_i / d x_k = y_i * (delta_ik - y_k)
      // matches the standard softmax. Thus:
      //   dL/dx_i = y_i * (dL/dy_i - sum_j y_j * dL/dy_j)
      SizeM1 := FOutput.Size - 1;
      Dot := 0;
      for i := 0 to SizeM1 do
