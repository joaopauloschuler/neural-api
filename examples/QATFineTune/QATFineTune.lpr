...
motivate QAT on a tiny synthetic image-classification task:
  ...
  2. PTQ    - the SAME trained weights after post-training int8 weight
              quantization (TNNet.QuantizeWeightsInt8). This usually drops
              accuracy versus float.
  3. QAT    - a network that carries TNNetFakeQuantize layers on its
              activations, initialised from the float weights and then
              fine-tuned for a few epochs so it learns to be robust to the
              quantization noise, recovering accuracy.

The dataset is generated synthetically (three easily separable textured
classes plus noise) so the example downloads nothing and runs in seconds.
...
const
  ...
  cTrainCount = 800;
  cTestCount  = 300;
  cNoise      = 2.2;  // heavy noise: keeps float accuracy moderate so that
                      // int8 weight rounding produces a visible PTQ drop.

// Builds one synthetic cImgSize x cImgSize x 1 example for the given class.
// Each class has a distinct deterministic pattern; uniform noise is added so
...
begin
  ...
  // int8 weight rounding (PTQ) costs accuracy and QAT can recover it.
  case ClassId of
...
begin
  ...
  // ---- 3. QAT: insert fake-quant layers, warm-start, fine-tune ---------
  // Same quantized topology, warm-started from the float weights, but now we
  // FINE-TUNE: the fake-quant rounding is in the forward pass and gradients
  // flow back through the straight-through estimator, so the weights adapt to
  // be robust to quantization. Observers calibrate during training; we freeze
  // them and int8-quantize the weights before measuring inference accuracy.
  WriteLn('QAT fine-tuning with TNNetFakeQuantize activations...');
  QATNet := BuildQATNet();
  QATNet.Layers[1].CopyWeights(FloatNet.Layers[1]); // ConvolutionReLU
  QATNet.Layers[4].CopyWeights(FloatNet.Layers[3]); // FullConnectReLU
  QATNet.Layers[6].CopyWeights(FloatNet.Layers[4]); // FullConnectLinear

  Fit := TNeuralFit.Create();
  Fit.Verbose := false;
  Fit.HideMessages();
  Fit.InitialLearningRate := 0.004;
  Fit.LearningRateDecay := 0;
  Fit.L2Decay := 0;
  Fit.Fit(QATNet, TrainPairs, nil, nil, {batchsize=}32, {epochs=}40);
  Fit.Free;
  ...
  WriteLn('PTQ   (int8 weights, no QAT)  ', (AccPTQ   * 100):6:2, ' %');
  WriteLn('QAT   (fake-quant fine-tune)  ', (AccQAT   * 100):6:2, ' %');
  WriteLn('===========================================================');
