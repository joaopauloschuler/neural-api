program OnlyTwoLayersXOrOperation;
(*
OnlyTwoLayersXOrOperation: this free pascal source code trains a neural network
that contains only two layers to learn the XOR boolean operation.
...
type
  ...
  TBackInput  = array[0..3] of array[0..1] of TNeuralFloat;  // Input data for XOR operation
  TBackOutput = array[0..3] of array[0..0] of TNeuralFloat;  // Expected output for XOR operation
...
begin
  ...
  NN.AddLayer(TNNetFullConnect.Create(2));
...
begin
  Application.Title:='Only Two Layers - XOR Operation';
  RunAlgo();
end.
