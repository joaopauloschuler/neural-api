program OnlyTwoLayersAbs;
(*
OnlyTwoLayersAbs: this free pascal source code trains a neural network
that contains only one neuron to learn the function f(x,y) = Abs(x + y).
...
type
  ...
  TBackInput  = array[0..1] of TNeuralFloat;  // Input data for Abs(x + y)
  TBackOutput = array[0..0] of TNeuralFloat;  // Expected output for Abs(x + y)
...
begin
  ...
  NN.AddLayer(TNNetFullConnectReLU.Create(2));         // Single neuron layer connected to both inputs from the previous layer.
  NN.AddLayer(TNNetFullConnectLinear.Create(1));       // Single neuron layer connected to both inputs from the previous layer.

  NN.SetLearningRate(0.00001, 0);                      // Set the learning rate and momentum

  pOutPut := TNNetVolume.Create(1, 1, 1, 1);           // Create a volume to hold the output

  WriteLn;

  for EpochCnt := 1 to 1000000 do
  begin
    ...
    vOutput[0] := Abs(vInputs[0] + vInputs[1]);        // Abs(X+Y)
...
begin
  Application.Title:='Only Two Layers - Abs(x+y)';
  RunAlgo();
end.
