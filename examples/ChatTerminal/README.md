...
| `--profile` | per-layer-class forward timing to **stderr** after each turn, one `[profile] prefill:` report (the windows on the `--prefill-window` twin and the single-token tail on the main net, each with its own table) and one for the decode steps, each followed by a `[sched]` line with the layer-graph scheduler stats (graph width, parallel vs serial passes, peak in-flight) | off |
...
| `--prefill-window N` | prefill the prompt N tokens per forward on a second, width-N copy of the net (`TChatEngine.WindowNN`); the state crosses to the width-1 net with a session snapshot before the tail and the decode loop. The tail that does not fill a window is fed one token at a time — nothing is padded. Costs a second copy of the weights in RAM and on the device and a second checkpoint load | 0 (one token per forward) |
