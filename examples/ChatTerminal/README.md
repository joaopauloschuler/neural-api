...
| `--stats` | per-turn timing to **stderr**: TTFT (prefill + first token), `prefill X tok/s` (prefilled tokens over TTFT minus one mean decode step; omitted when nothing was prefilled), steady-state decode tok/s, and `prompt N (reused K)` from the KV-cache reuse | off |
