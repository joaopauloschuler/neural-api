...
block: a `Dense -> GEGLU -> Dense` sandwich trained on a synthetic
regression target. No attention, no embedding, no positional
encoding. Just `TNNetGEGLU` wired between two fully-connected layers
so you can see the gated activation work in isolation.

## What it shows

The model is the canonical "GEGLU FFN" from the gated-activations
...
TNNetInput(4)                            # 4 raw features
  ...
  -> TNNetGEGLU                          # value * GELU(gate) -> depth 16
  -> TNNetFullConnectLinear(1)           # regression head
```

`TNNetGEGLU` splits its input depth in half: the second half is
passed through the GELU activation and used to gate the first
...
learnable by a single GEGLU FFN block.

## Build & run

```
lazbuild GEGLUFeedForward.lpi
../../bin/x86_64-linux/bin/GEGLUFeedForward
```

Or directly with fpc:

```
fpc -dRelease -dUseCThreads -O3 -Fu../../neural GEGLUFeedForward.lpr
./GEGLUFeedForward
```
...
Layer 2 ... TNNetGEGLU          Output:1,1,16
Layer 3 ... TNNetFullConnectLinear Output:1,1,1

Training Dense -> GEGLU -> Dense FFN on synthetic regression...
