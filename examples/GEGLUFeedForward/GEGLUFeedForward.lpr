program GEGLUFeedForward;
(*
GEGLUFeedForward: a tiny transformer-style feed-forward block
demo. Trains the canonical "Dense -> GEGLU -> Dense" sandwich on a
synthetic regression task so you can see GEGLU end-to-end in
isolation, without an attention block or embedding around it.

Pipeline:
  TNNetInput(D_in)
    -> TNNetFullConnectLinear(2*D_hidden)   { gate || value packed on depth }
    -> TNNetGEGLU                           { value * GELU(gate), halves depth }
...
const
  ...
  begin
    ...
    // Dense(2*D_hidden) -> GEGLU -> Dense(D_out) in a single call.
    NN.AddGEGLUFeedForward(cDIn, cDHidden, cDOut);

    WriteLn('Layers:');
    NN.DebugStructure();

    WriteLn('Training Dense -> GEGLU -> Dense FFN on synthetic regression...');
...
begin
  Application.Title:='GEGLU Feed-Forward Example';
  RunAlgo();
end.
