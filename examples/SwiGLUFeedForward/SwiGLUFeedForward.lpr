program SwiGLUFeedForward;
(*
SwiGLUFeedForward: a tiny transformer-style feed-forward block
demo. Trains the canonical "Dense -> SwiGLU -> Dense" sandwich on a
synthetic regression task so you can see SwiGLU end-to-end in
isolation, without an attention block or embedding around it.

Pipeline:
  TNNetInput(D_in)
    -> TNNetFullConnectLinear(2*D_hidden)   { gate || value packed on depth }
    -> TNNetSwiGLU                          { value * Swish(gate), halves depth }
...
const
  ...
  begin
    ...
    WriteLn('Training Dense -> SwiGLU -> Dense FFN on synthetic regression...');
...
begin
  Application.Title:='SwiGLU Feed-Forward Example';
  RunAlgo();
end.
